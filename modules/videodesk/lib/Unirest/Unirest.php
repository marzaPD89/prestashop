...
class Unirest
{
	...
	/**
	 * Send a FTP request to a URL
	 * @param  string $url     URL to send the GET request to
	 * @param  array  $headers Additional headers to send
	 * @return string|stdObj   Response string or stdObj if response is json-decodable
	 */
	public static function ftp($url, $headers = array(), $params)
	{
		return Unirest::request(HttpMethod::FTP, $url, $params, $headers);
	}
	 ...
	 * @return \Unireset\HttpRestResponse 	\Unirest\HttpRestResponse object
	 */
	private static function request($httpMethod, $url, $body = NULL, $headers = array())
	{
		...
		$ch = curl_init();
		if ($httpMethod == HttpMethod::FTP) {
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $body['command']);
			curl_setopt($ch, CURLOPT_USERPWD, $body['username'].":".$body['password']);
		}
		elseif ($httpMethod != HttpMethod::GET) {
		...
		curl_setopt ($ch, CURLOPT_CAINFO, dirname(__FILE__).'../../config/videodesk.com.crt');
		...
		return new HttpRestResponse($httpCode, $body, $header);
	}
	
	
// 	$curl = curl_init();
// // 	curl_setopt($curl, CURLOPT_URL, "ftp://".$ftp_host);
