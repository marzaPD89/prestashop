...
class VideodeskConfiguration
{
	...
	/**
	 * Display of the first connection home
	 */
	public function processDisplayHome()
	{
		...
		$account_exists = $this->videodeskEmailExist($employee->email);
	...
	/**
	 * Display of the home once at least one Website ID is configured
	 */
	public function processDisplayConfiguration()
	{
		...
		$smarty->assign(array(
			...
			'VD_BO_CONF' => $currentIndex.'&token='.$token.'&configure=' . $this->module->name . '&tab_module=' . $this->module->tab . '&module_name=' . $this->module->name,
	...
	public function processDisplayShopConfiguration()
	{
		...
		$smarty->assign(array(
			'token_videodesk' => $token,
			'path_module' => _PS_MODULE_DIR_ . $this->module->name . '/14/views/templates/admin/',
			'baseDir' => $this->module->getPathUri(),
			'img_dir' => $this->module->getPathUri() . 'views/img/',
			'id_shop' => $id_shop,
			...
			'return_url' => $currentIndex.'&token='.$token.'&configure=' . $this->module->name . '&tab_module=' . $this->module->tab . '&module_name=' . $this->module->name,
	...
	/**
	 * Validate the Website IDs upon Videodesk and update configuration
	 */
	public function processSignIn()
	{
		...
		if ($website_configured) {
			Configuration::updateValue('VD_CONF_STATE', 'configured');
		}
		else {
		...
		$smarty->assign('website_id', Tools::getValue('website_id'));
	...
	/**
	 * Check the validity of a Website ID
	 * 
	 * @param int $id_shop
	...
	public function checkWebsiteId($id_shop, $website_id)
	{
		...
		if ($website_id != '') {
			...
			$posted_data = array_merge($this->getConfPost(), array(
				'website_id' => $website_id
			));
			if (Configuration::get('VD_CONNECTION_SECURED') == '1') {
				$response = Unirest::post($this->buildUrl(Configuration::get('VD_FORM_SIGNIN')), array(
				...
				if ($response->code == "200") {
					...
					$this->_errors[] = $this->module->l('Unable to connect videodesk server');
				}
				else {
				...
				$response = HttpRequester::post($this->buildUrl(Configuration::get('VD_FORM_SIGNIN')), $posted_data);
	...
	public function videodeskEmailExist($email)
	{
		...
		if ($email != '') {
			...
			$response = HttpRequester::post(Configuration::get('VD_EMAIL_CHECK'), $posted_data);
			if ($response->getStatus() == "200") {
				$content = $response->asJson();
				if ($content->exist)
					return true;
	...
	public function validateWebsiteId($id_shop, $content, $merchant_code)
	{
		if (isset($content->valid) && $content->valid) {
			...
			if ($pos !== false) {
				...
				$configured_shops = VideodeskShopConfiguration::getWebsiteIds();
				if (count($configured_shops) > 0) {
					$action = 'update';
					$vd_shop_configuration->id = $id_shop;
				}
				else {
					$action = 'add';
					$vd_shop_configuration->id = $id_shop;
				...
				$vd_shop_configuration->display_for_all = 1;
				$vd_shop_configuration->track_stats = 1;
	
				if (Configuration::get('VD_CONNECTION_SECURED') == '1') {
					$vd_shop_configuration->ftp_active = 1;
				...
				if ($action == 'add' && $vd_shop_configuration->add()) {
					return true;
				}
				elseif ($action == 'update' && $vd_shop_configuration->update()) {
					return true;
				}
				else {
	...
	public function processDisplaySignUpStep1()
	{
		...
		try {
			$response = HttpRequester::post(
				$this->buildUrl(Configuration::get('VD_FORM_SIGNUP_ACCOUNT')),
				$this->getConfPost());
			if ($response->getStatus() == "200") {
				$content = $response->asJson();
				if (is_array($content)) {
			...
			$shop_fields = null;
			$response = HttpRequester::post(
				$this->buildUrl(Configuration::get('VD_FORM_SIGNUP_SHOP')),
				$this->getConfPost());
			if ($response->getStatus() == "200") {
				$content = $response->asJson();
				if (is_array($content)) {
		...
		if ($account_fields != null && $shop_fields != null) {
			...
			$smarty->assign(array(
				...
				'form_action' => Configuration::get('VD_FORM_SIGNUP_VALIDATION')
			));
		}
		...
		$smarty->assign(array(
			...
			'token_videodesk' => Tools::getAdminTokenLite('AdminModules'),
	...
	public function processDisplaySignUpStep2()
	{
		...
		$id_current_employee = $cookie->id_employee;
	...
	public function processDisplaySignUpStep3()
	{
		...
		try {
			...
			$response = HttpRequester::post(
				$this->buildUrl(Configuration::get('VD_FORM_SIGNUP_AGENT')),
				$conf);
			if ($response->getStatus() == "200") {
				$content = $response->asJson();
			...
			foreach ($websites as $id_shop => $website_id) {
				...
				$response = HttpRequester::post(
					$this->buildUrl(Configuration::get('VD_FORM_SIGNUP_SHOP_AGENT')),
					$conf);
				if ($response->getStatus() == "200") {
					$content = $response->asJson();
	...
	public function processUpdateShopConfiguration()
	{
		...
		if (!empty($id_shop)) {
			...
			VideodeskShopConfigurationCriteriaValue::DeleteAllForShop($id_shop);
			foreach ($criterias as $key => $value) {
				$criteria = new VideodeskShopConfigurationCriteria($key);
				$criteria_value = new VideodeskShopConfigurationCriteriaValue($key, $id_lang, $id_shop);
				$criteria_value->id_shop = $id_shop;
// 				$criteria_value->id = $key;
	...
	public function buildFormStep1($account_fields, $shop_fields, $params = null)
	{
		...
		$uid = Configuration::get('VD_ACCOUNT_UID');
		...
		if ($uid == false) {
			...
			foreach ($account_fields as &$field) {
				...
				if (isset($field->mapping)) {
					$this->formFieldMapping($field, null, array(
				...
				if ($field->id == 'is_accept') {
					$field->caption = ' <a href="' . $this->buildUrl(Configuration::get('VD_CGU')) . '" class="cgu" target="_blank">' . $field->caption . '</a>';
		...
		foreach ($shop_fields as $key_field => $field) {
			...
			if (isset($shop_field->isLogo) && $shop_field->isLogo) {
				...
				$field_logo_url->value = (Configuration::get('PS_SSL_ENABLED', null, null) ? 'https://' : 'http://') . Tools::getShopDomain() . '/img/' . $shop_field->value;
			...
			elseif ($field->id == 'is_use_videodesk') {
				...
				$shop_field->hidden = 'hidden';
				//$shop_field->caption .= " ".$shop['name'];
				$shop_field->caption = null;
				$field_tmp->html = array(
					$shop_field
				);
			}
	...
	public function buildFormStep3($agent_fields, $websites_form, $params = null)
	{
		...
		foreach ($employee_shops as $id_employee => $employees) {
			...
			$employee_lang_iso = Language::getIsoById($employee->id_lang);
			...
			if ($id_employee == $cookie->id_employee) {
				$current_employee = true;
				$uid = Configuration::get('VD_ACCOUNT_UID_AGENT');
			}
			else {
			...
			foreach ($agent_fields as &$field) {
				...
				if ($agent_field->id == 'language') {
					...
					foreach ($agent_field->options as $value => $label) {
						...
						if (strtolower($value) == strtolower($employee_lang_iso)) {
							$checked = true;
...
// 				$field_shop_title->html = $employee_name . ' - ' . $shop->name . "<span>&nbsp;</span>";
				$field_shop_title->html = "<span>&nbsp;</span>";
...
// 										break;
					...
					if (isset($shop_field)) {
						$field_tmp = $this->formFieldInit();
						$field_tmp->html = array($shop_field);
						$shop_form[] = $field_tmp;
					}
				}
	...
	/**
	 * Update the general Videodesk Configuration at the module installation
	 * 
	 * @return boolean
	 */
	public function updateConfiguration()
	{
		...
		$response = HttpRequester::post($this->buildUrl($this->module->_videodesk_main_url), $this->getConfPost());
		if ($response->getStatus() == "200") {
			$content = $response->asJson();
			...
			// Notification URLs
			Configuration::updateValue('VD_NOTIF_VDCOM', $config->notification_url->videodesk);
			Configuration::updateValue('VD_NOTIF_VDPSP', $config->notification_url->prestashop);
			// Standard links
			Configuration::updateValue('VD_READMORE', $config->readmore_url);
			Configuration::updateValue('VD_PRICING', $config->pricing_url);
			Configuration::updateValue('VD_CGU', $config->cgu_url);
			// Goal notification for statistics
			Configuration::updateValue('VD_NOTIF_OBJECTIVE', $config->validate_command);
			// Edito
			Configuration::updateValue('VD_EDITO_URL', $config->edito_iframe->url);
			Configuration::updateValue('VD_EDITO_WIDTH', $config->edito_iframe->width);
			Configuration::updateValue('VD_EDITO_HEIGHT', $config->edito_iframe->height);
			// Preview
			Configuration::updateValue('VD_HELP_SIGNUP_URL', $config->signup_help_iframe->url);
			Configuration::updateValue('VD_HELP_SIGNUP_WIDTH', $config->signup_help_iframe->width);
			Configuration::updateValue('VD_HELP_SIGNUP_HEIGHT', $config->signup_help_iframe->height);
			// Module in Back Office
			Configuration::updateValue('VD_BO_MODULE_JS', $config->backoffice_module_url);
			// Preview
			Configuration::updateValue('VD_PREVIEW_URL', $config->preview_url);
			// BO Videodesk URLs
			Configuration::updateValue('VD_BO_HOME', $config->backoffice_url->home);
			Configuration::updateValue('VD_BO_TEMPLATE', $config->backoffice_url->template);
			Configuration::updateValue('VD_BO_TEXTS', $config->backoffice_url->texts);
			Configuration::updateValue('VD_BO_MESSAGES', $config->backoffice_url->pretyped_messages);
			Configuration::updateValue('VD_BO_AGENT', $config->backoffice_url->agent);
			// Interfaces
			$interfaces = $content->interfaces;
			// Email - Videodesk account exists with this email address
			Configuration::updateValue('VD_EMAIL_CHECK', $interfaces->email_check);
			// Sign In - Website Id validation
			Configuration::updateValue('VD_FORM_SIGNIN', $interfaces->signin);
			// Sign Up - General informations
			Configuration::updateValue('VD_FORM_SIGNUP_ACCOUNT', $interfaces->signup->account_form);
			Configuration::updateValue('VD_FORM_SIGNUP_SHOP', $interfaces->signup->shop_form);
			Configuration::updateValue('VD_FORM_SIGNUP_ACCOUNT_SUB', $interfaces->signup->account_form_submit);
			Configuration::updateValue('VD_FORM_SIGNUP_SHOP_SUB', $interfaces->signup->shop_form_submit);
			// Sign Up - Agents
			Configuration::updateValue('VD_FORM_SIGNUP_AGENT', $interfaces->agent_signup->agent_form);
			Configuration::updateValue('VD_FORM_SIGNUP_AGENT_SUB', $interfaces->agent_signup->agent_form_submit);
			Configuration::updateValue('VD_FORM_SIGNUP_SHOP_AGENT', $interfaces->agent_signup->shop_agent_form);
			Configuration::updateValue('VD_FORM_SIGNUP_SHOP_AGENT_SUB', $interfaces->agent_signup->shop_agent_form_submit);
	...
	public function buildUrl($url)
	{
		...
		return $url . $prefix . 'lang=' . Language::getIsoById((int) $cookie->id_lang) . '&platform=' . $this->module->_platform . '&version=' . $this->module->version;
	}
	
	public function buildForm($fields, $params = null)
	{
		global $cookie;
		$form = array();
		...
		foreach ($fields as &$field) {
			...
			$field_tmp->type = "p";
			$field_tmp->class = "text";
			
			// Pre-fill values of the form
			if (isset($field->mapping)) {
				// Shop e-mail
				if ($field->mapping == "shop_email") {
					$field->value = Configuration::get('PS_SHOP_EMAIL');
				}
				// Country list
	...
	{
		return VideodeskShopConfiguration::getConf();
		/*
		 * modif suppr pour 1.4
		 */
	}
	
	/**
	 * Returns the progress of Customization
	 * 
	 * @return number
	 */
	public static function getProgress()
	{
		$sql = 'SELECT (100 / ' . self::$_nbStep . ') * (progress_criterias + progress_colors + progress_texts + progress_messages + progress_agent) FROM ' . _DB_PREFIX_ . 'videodesk_shop_configuration';
	...
	public function getCategoriesTree($id_category = null)
	{
		...
		if (!empty($id_category)) {
			...
			foreach ($childrenArray as $child_key => $child) {
				$category = new Category($child['id'], $id_lang);
	...
	/**
	 * Check if PrestaShop i Multishop => never in 1.4.X
	 * @return boolean
	 */
	...
	/**
	 * Returns the template file to display
	 * 
	 * @return string
	 */
	...
	/**
	 * Returns if the process needs display or backend process
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Returns if the process needs dForm lib to be included
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Send the notifications for each source, errors are hidden
	 * 
	 * @return boolean
	 */
		...
		if (is_array($this->module->_source) && !empty($this->module->_source)) {
			foreach ($this->module->_source as $key => $source) {
				...
				if (!$isSent) {
					...
					$url = $this->buildNotificationUrl(Configuration::get('VD_NOTIF_' . $source));
					try {
						$response = HttpRequester::get($url);
						
						if ($response->getStatus() == "200") {
							Configuration::updateValue('VD_NOTIF_SENT_' . $source, 1);
						}
					}
					catch (Exception $ex) {
						return true;
	...
	/**
	 * Add the mandatory parameters when call of a notification URL
	 * 
	 * @param string $url
	 * @param string $prestashop
	 * @return string
	 */
	public function buildNotificationUrl($url, $prestashop = null)
	{
		global $cookie;
		...
		$url .= 'firstname=' . urlencode($employee->firstname);
		$url .= '&lastname=' . urlencode($employee->lastname);
		$url .= '&email=' . urlencode($employee->email);
		...
		$url .= '&name0=' . urlencode(Configuration::get("PS_SHOP_NAME"));
		$url .= '&url0=' . urlencode(tools::getShopDomain(true));
		
		return $url;
	}
	
	/**
	 * Build the conf fields to include in each form
	 * 
	...
	/**
	 * Initiate a field structure for dForm
	 * 
	 * @param string $type
	 * @param string $class
	 * @return stdClass
	...
	/**
	 * Prefill fields with PrestaShop values
	 * 
	 * @param object $field
	 * @param int $id_shop
	...
	private function formFieldMapping($field, $id_shop = null, $employee = null)
	{
		...
		elseif ($field->mapping == "shop_name") {
			if ($id_shop != null) {
				...
				$field->value = Configuration::get('PS_SHOP_NAME');
		}
		// Shop phone
		...
		elseif ($field->mapping == "shop_main_url") {
			$field->value = (Configuration::get('PS_SSL_ENABLED', null, null, $id_shop) ? 'https://' : 'http://') . Tools::getShopDomain();
		}
		// Shop test URL
		...
		// Country list
		elseif ($field->mapping == "country_list") {
			$countries = Country::getCountries($lang->id);
			$country_select = array();
			foreach ($countries as $country) {
				if (strtolower($country['iso_code']) == strtolower($lang->iso_code))
	...
	public function getShops($shop_context = null)
	{
		...
		$shops_group_key = 0;
		$key = 0;
		$shops[$shops_group_key]['shops'][$key]['id_shop'] = 1;
		$shops[$shops_group_key]['shops'][$key]['id_shop_group'] = 1;
		$shops[$shops_group_key]['shops'][$key]['name'] = Tools::getShopDomain(false);
			...
			foreach ($shops as $shops_group_key => $shops_group) {
				if (!empty($shops_group['shops']) && is_array($shops_group['shops'])) {
					foreach ($shops_group['shops'] as $key => $shop) {
						$sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'videodesk_shop_configuration AS c WHERE c.id_conf = ' . $shop['id_shop'];
