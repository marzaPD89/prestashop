...
switch (Tools::getValue('signup_action')) {
	...
	case 'submitSignUp_Step1':
			...
			try {
				...
				if ($response->code == "200") {
					...
					if ((count($errors) > 0)) {
						...
						$uid = $content->uid;
						Configuration::updateValue('VD_ACCOUNT_UID', $uid);
						$uid_agent = $content->uid_agent;
						Configuration::updateValue('VD_ACCOUNT_UID_AGENT', $uid_agent);
					}
				}
				else {
					die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array($default_error))));
				}
			}
			catch (Exception $ex) {
				die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array($default_error))));
				...
				if ($response->code == "200") {
					...
					// Store the Website configuration : uid and FTP authentication
						...
						if (isset($content)) {
							foreach ($content as $id_shop => $shop) {
								if (is_object($shop)) {
									...
									if (!$vd_shop_configuration->add()) {
										die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array($module->l('Unable to store shop configuration')))));
									}
						...
						if ($website_configured) {
							...
							die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array($module->l('No shop was configured to be active on videodesk')))));
						}
		...
		die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array($module->l('Error occured, please try again')))));
    	break;
	...
	case 'submitSignUp_Step3':
		...
		$uid_agent = Configuration::get('VD_ACCOUNT_UID_AGENT');
		...
		if ($uid_agent === false || $uid_agent == null) {
			...
			$posted_data = array_merge($data, array('conf' => Tools::getValue('conf')));
			try {
				$response = Unirest::post(
		...
		if ($uid_agent !== false) {
			...
			foreach ($agent_shops as $id_shop => $agent_shop) {
				$shoperrors = array();
// 				$id_shop = str_replace("shop", "", $id_shop);
				$id_shop = 1;
				
				$agent_shop["uid_site"] = VideodeskShopConfiguration::getWebsite_idByIdShops($id_shop);
				...
				if ($response->code == "200") {
					...
					$shoperrors = Tools::jsonEncode(array('hasError' => 1, 'errors' => array($default_error)));
				}
			}
			if (!empty($shoperrors)) {
				die(($shoperrors));
				...
				Configuration::updateValue('VD_ACCOUNT_UID_AGENT', null);
				
				$id_last_employee = end(array_keys($datas));
				if ($id_current_employee == $id_last_employee) {
					$action = array('action' => 'success');
				}
				else {
					$found = false;
					reset($datas);
					foreach($datas as $id_employee => $data) {
						if ($found) {
							$action = array('action' => 'next', 'id_employee' => $id_employee);
							break;
						}
				...
				die(Tools::jsonEncode(array_merge(array('hasError' => 0, 'content' => $content), $action)));
			}
		}
    		 
    default:
    	die(Tools::jsonEncode(array('hasError' => 1, 'errors' => array('Action not recognized'))));
    	exit;
}
exit;