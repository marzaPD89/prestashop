...
<div id="videodesk_calls" {if $type == 'customer'}style="width: 50%;"{/if}>
	<input type="hidden" name="vd_token" id="vd_token" value="{$token}" />
        ...
        {else}
        	...
        	{/if}
		{/if}  
        <table width="{if $type == 'order'}300px{else}100%{/if}" cellspacing="0" cellpadding="0" id="videodesk_calls_table" class="table">
        <colgroup>
                <col width="25%">
                <col width="15%">
                ...
                <tbody>
                    {foreach from=$calls key=k item=call}
                        <tr id="call{$call.call->id}">
                            ...
                            {if $type == 'customer'}<td>{if isset($call.call->id_cart)}<a href="index.php?tab=AdminCarts&id_cart={$call.call->id_cart}&viewcart&token={$token_cart}"><img src="../img/admin/details.gif" /></a>{/if}</td>{/if}
                            <td>{$call.employee->firstname} {$call.employee->lastname}</td>
                            <td>{$call.call->connexion_page}</td>
                            <td><span style="text-decoration:underline; cursor:pointer;" class="detail" id="{$call.call->id}" ><img src="../img/admin/details.gif" alt="{l s='See Detail' mod='videodesk'}"></span></td>
                        </tr>
                    {/foreach}
                </tbody>
        </table>
