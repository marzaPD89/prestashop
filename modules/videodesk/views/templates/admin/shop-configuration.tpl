        ...
        <form action="{$return_url}&submitShopConfiguration" method="post">
         ...
         <div class="block criterias clearfix clearBoth">
            ...
            <div class="criteria clearfix">
                ...
                <input class="text" name="criteria_value[{$criteria.id_criteria}]" id="criteria_value_{$criteria.id_criteria}" value="{$criteria.value}" style="text-align: center;" /> <label for="criteria_value_{$criteria.id_criteria}">{l s=$criteria_value mod='videodesk' sprintf=$currencySign}</label>
                {/if}
            </div>
            {/foreach}
        </div>
